...
import org.ambient.flink.streaming.connectors.redis.common.config.FlinkJedisSentinelConfig;
import org.ambient.flink.streaming.connectors.redis.common.container.RedisCommandsContainer;
import org.ambient.flink.streaming.connectors.redis.common.container.RedisCommandsContainerBuilder;
...
public class RedisSentinelClusterTest extends TestLogger {
    ...
    private static final List<Port> sentinalGenerators = Arrays.asList(getAvailablePort(), getAvailablePort());
    private static final List<Port> groupGenerators = Arrays.asList(getAvailablePort(), getAvailablePort());

    private static final List<Integer> sentinels = Arrays.asList(
            sentinalGenerators.get(0).getPort(), sentinalGenerators.get(1).getPort());
    private static final List<Integer> group1 = Arrays.asList(groupGenerators.get(0).getPort(),
            groupGenerators.get(1).getPort());
    ...
    public static void tearDownCluster() {
        try {
            ...
            for(Port portGenerator : sentinalGenerators) {
                portGenerator.close();
            }
            for(Port portGenerator : groupGenerators) {
                portGenerator.close();
            }
        } catch (Exception e) {}
    }
